function out = detect_LFP_burst(X, freq_band, Num_cycle, separating_cycle, Fs, varargin)
% Inputs:
...
%   'DensityWindow;      _ Convolving window for burst density 
%
% Outputs:
...
%***********************************
% Kianoush Banaie Boroujeni (Kia) - 2024
% Banaie Boroujeni et al., (2025) Fast Attentional Information Routing via High-Frequency 
% Bursts in the Human Brain
%***********************************
...
addParameter(p, 'DensityWindow', 0.5, @(x) isnumeric(x) && x > 0);
parse(p, varargin{:});

X = X(:);
filter_order = p.Results.FilterOrder;
burst_duration_cycles = p.Results.BurstDurationCycles;
smooth_window = p.Results.DensityWindow;
...
baseline_percentile = 25;  %  25th percentile for baseline estimate
IE_baseline = prctile(IE_t, baseline_percentile);

IE_median = median(IE_t);
MAD = median(abs(IE_t - IE_median));
robust_std = 1.4826 * MAD;  % Scale factor to match normal distribution

threshold_multiplier = 2.5;  
threshold = IE_baseline + threshold_multiplier * robust_std;

percentile_threshold = prctile(IE_t, 75);  % 75th percentile
threshold = min(threshold, percentile_threshold * 1.5);  % Use the lower of the two

% Calculate an Amplitude Threshold
[amp_peaks, ~] = findpeaks(a_t);
if ~isempty(amp_peaks)
    
    amp_threshold = median(amp_peaks);
else
    amp_threshold = median(a_t) * 2; 
end
...
IE_boundary_threshold = IE_baseline * 1.5;  
...
for i = 1:num_bursts
    ...
    if length(indices) >= min_burst_duration_samples
        ...
        for idx = peak_idx:-1:indices(1)
            if IE_t(idx) < IE_boundary_threshold || delta_omega_t(idx) > mean_delta_omega + 2 * std_delta_omega
                start_idx = idx + 1;
                break;
            end
        end
...
window_length = round(smooth_window * Fs);  
gauss_window = gausswin(window_length, 2.5);
gauss_window = gauss_window / max(gauss_window);
raw_conv = conv(burst_events, gauss_window, 'same');
norm_factor = conv(ones(size(burst_events)), gauss_window, 'same');
norm_factor = norm_factor / max(norm_factor);
burst_density = raw_conv ./ norm_factor;
